...
function FloatingLines({
  ...
  // 性能优化状态
  const isVisibleRef = useRef(true); // Tab 可见性
  const isInViewRef = useRef(false); // 滚动视图可见性
  ...
  useEffect(() => {
    ...
    // 1. Tab Visibility Setup
    const handleVisibilityChange = () => {
      isVisibleRef.current = !document.hidden;
    };
    document.addEventListener("visibilitychange", handleVisibilityChange);

    // 2. Intersection Observer Setup (关键性能优化)
    const observer = new IntersectionObserver(
      ...
      { root: null, threshold: 0 } // 只要有一点点可见就开始渲染
    );
    ...
    const renderer = new WebGLRenderer({
      antialias: false, // 对于这种 Shader 效果，关闭 AA 可以提升性能且影响不大
      alpha: false,
      powerPreference: "high-performance",
    });

    // 强制限制 DPR，高分屏下最多 1.5，普通屏 1.0，防止 4K/Retina 爆炸
    ...
    let raf = 0;
    let lastTime = 0;
    const frameInterval = 1000 / 60; // 锁定 60fps

    const renderLoop = (time: number) => {
      raf = requestAnimationFrame(renderLoop);

      // 核心优化：如果 Tab 不可见 或者 元素滚出屏幕，完全跳过渲染
