...
export interface GridScanProps {
   ...
   * Values < 1.0 significantly improve performance.
   * Default: 1.0
   */
...
const GridScan = ({
  ...
  // 性能优化 Ref
  const isVisibleRef = useRef(true); // Tab
  const isInViewRef = useRef(false); // Scroll
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({
      antialias: false, // 后处理开启时，AA通常由后处理或分辨率决定，关闭节省资源
      alpha: true,
      powerPreference: "high-performance",
    });
    rendererRef.current = renderer;

    // 强制限制像素比，过高会导致 PostProcessing 极慢
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5);
    renderer.setPixelRatio(dpr);

    // 应用分辨率缩放
    const targetWidth = container.clientWidth * resolutionScale;
    const targetHeight = container.clientHeight * resolutionScale;
    renderer.setSize(targetWidth, targetHeight, false); // false = updateStyle false, 我们让 CSS 负责拉伸
    ...
    // 手动设置 Canvas 样式以匹配容器大小（因为上面 setSize 的第三个参数是 false）
    renderer.domElement.style.width = "100%";
    renderer.domElement.style.height = "100%";

    const uniforms = {
      iResolution: {
        value: new THREE.Vector3(targetWidth, targetHeight, dpr),
      },
      iTime: { value: 0 },
    ...
    if (enablePost) {
      composer = new EffectComposer(renderer, {
        frameBufferType: THREE.HalfFloatType, // 使用半浮点纹理，通常足够且更快
      });
    ...
    const onResize = () => {
      const w = container.clientWidth * resolutionScale;
      const h = container.clientHeight * resolutionScale;
    ...
    const tick = () => {
      // 核心优化点：
      // 1. Tab 不可见 (document.hidden) -> 停止
      // 2. 组件滚出屏幕 (!isInViewRef.current) -> 停止
      if (!isVisibleRef.current || !isInViewRef.current) {
        rafRef.current = requestAnimationFrame(tick);
        // 重置时间戳，防止再次进入视图时时间跳跃过大
  ...
  return (
    <div
      ref={containerRef}
      className={`gridscan w-full h-full ${className || ""}`}
      style={style}
    />
  );
};
